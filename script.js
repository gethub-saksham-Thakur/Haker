...
function onKey(a) {
    ...
    overlayShown || ("Shift" === a.key && shiftCount++,
    "Alt" === a.key && altCount++,
    3 <= altCount && (altCount = 0,
    showAlert(granted)),
    3 <= shiftCount && (shiftCount = 0,
    showAlert(denied)),
    type(speed))
}
function type(a) {
...
function onSpeedChange(a) {
    if (/\d+/.test(a.target.value)) {
        ...
        if (b < speed_range.min || b > speed_range.max)
            return;
        speed_range.value = b,
        speed_input.value = b,
        speed = b,
...
function onFontSizeChange(a) {
    if (/\d+/.test(a.target.value)) {
        let b = parseInt(a.target.value);
        if (b < font_size_range.min || b > font_size_range.max)
            return;
        font_size_range.value = b,
        font_size_input.value = b,
        setFontSize(b),
        setStore("font_size", b)
    }
}
function onFontChange(a) {
    setFont(a.target.value),
    setStore("font", a.target.value)
}
...
function setThemeColor(a) {
    ...
    for (const b of document.querySelectorAll(".theme_border_color"))
        b.style.borderColor = a;
    for (const b of document.querySelectorAll(".theme_color"))
        b.style.color = a;
    for (const b of document.querySelectorAll(".theme_bg_color"))
        b.style.background = a;
    for (const b of document.querySelectorAll(".theme_fill_color"))
        b.style.fill = a;
    setStore("color", a)
}
function setFont(a) {
    const b = a.replace(/\s/, "+");
    font_div.innerHTML = `<link href="${`https://fonts.googleapis.com/css?family=${b}&display=swap`}" rel="stylesheet" />`,
    typer.style.fontFamily = a
}
function setFontSize(a) {
    typer.style.fontSize = a + "px",
    cursor.style.fontSize = a + "px"
}
